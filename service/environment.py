...
def main():
    ...
    for spots in alls:
      ...
      for set,t,r,m in zip(spots,tt,rr,mm):
        uid = set['uid']
        gid = set['gid']
        lon = set['lon']
        lat = set['lat']
        sql_cmd = '''UPDATE lands_%(uid)s SET temperature = %(t)f, rainfall = %(r)f, moisture = %(m)f , updated = NOW() WHERE gid = %(gid)d AND lon = %(lon)d AND lat = %(lat)d ;
        ''' % locals()
        print (sql_cmd)
